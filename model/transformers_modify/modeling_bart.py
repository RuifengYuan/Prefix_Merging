...
class BartEncoder(nn.Module):
    ...
    def forward(
        self, input_ids, attention_mask=None, past_key_values=None,
            output_attentions=True, output_hidden_states=False, return_dict=False
    ):
        """
        Args:
...
class BartForConditionalGeneration(PretrainedBartModel):
    ...
    def __init__(self, config: BartConfig, set_preseqlen=20):
        super().__init__(config)

        config.add_use_prefix()
        config.add_preseqlen(set_preseqlen)
        self.bartconfig=config
    ...
    def set_preseqlen(self, p):
        self.bartconfig.add_preseqlen(p)        
